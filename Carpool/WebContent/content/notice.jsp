	...
	<table id="boardTable">
		...
		<tr>
			<c:if test="${sessionScope.m.type == 0}">
				...
				<a href="<%=request.getContextPath()%>/content/noticeWriteForm.jsp">
				<img src="${pageContext.request.contextPath }/img/write.png"></a>
				</td>
			</c:if>
		</tr>
		<tr>
						...
						<c:when test="${!empty list }">
							<c:forEach var="b" items="${list }" varStatus="status">
								<tr>
									<td>${b.num}</td>
									<td>${b.writer}</td>
									<td><a href="${pageContext.request.contextPath }/Board2NoticeDetailController?num=${b.num }">${b.title}</a></td>
									<td>${b.w_date}</td>
								</tr>
							</c:forEach>
						</c:when>
